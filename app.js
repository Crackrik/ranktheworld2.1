...
class RankEverything {
    ...
    async init() {
        ...
        console.log('Dataset loaded:', this.dataset.length, 'items');
        this.organizeCategories();
        console.log('Categories organized:', Object.keys(this.categories).length, 'categories');
    ...
    organizeCategories() {
        ...
        // Initialize ELO ratings (or load from saved)
        this.dataset.forEach(item => {
            if (!this.rankings[item.id]) {
    ...
    loadRankings() {
        const stored = localStorage.getItem('rankeverything_rankings');
        return stored ? JSON.parse(stored) : {};
    }
    
    saveRankings() {
        localStorage.setItem('rankeverything_rankings', JSON.stringify(this.rankings));
    }
    
    loadComparisons() {
        const stored = localStorage.getItem('rankeverything_comparisons');
        return stored ? JSON.parse(stored) : [];
    }
    
    saveComparisons() {
        localStorage.setItem('rankeverything_comparisons', JSON.stringify(this.comparisons));
        this.updateStats();
    }
    ...
    setupEventListeners() {
        ...
        // Item card clicks - use arrow functions to preserve 'this' context
    ...
    startRanking() {
        // Don't reset progress - it tracks session comparisons for display only
        // Total comparisons tracked in this.comparisons.length
        this.updateProgress();
    ...
    selectItem(winner) {
        console.log('selectItem called with:', winner);
        if (!this.currentPair) {
            console.error('No current pair!');
            return;
        }
        ...
        console.log('Winner:', winnerItem.title, 'Loser:', loserItem.title);
        ...
        console.log('Total comparisons now:', this.comparisons.length);
    ...
    updateProgress() {
        // Show total all-time comparisons
        const totalAllTime = this.comparisons.length;
        document.getElementById('progress-text').textContent = `${totalAllTime} TOTAL COMPARISONS`;
        
        // Update total comparisons stat
        document.getElementById('total-comparisons').textContent = totalAllTime.toLocaleString();
        
        // Animate progress bar (cycles every 20 comparisons for visual feedback)
        const percent = ((totalAllTime % 20) / 20) * 100;
        document.getElementById('progress-bar').style.width = `${percent}%`;
    }
    
    updateStats() {
        document.getElementById('total-items').textContent = this.dataset.length.toLocaleString();
        document.getElementById('total-comparisons').textContent = this.comparisons.length.toLocaleString();
    }
...
function resetAllData() {
    if (confirm('This will DELETE all your rankings and comparisons. Are you sure?')) {
        localStorage.removeItem('rankeverything_rankings');
        localStorage.removeItem('rankeverything_comparisons');
        localStorage.removeItem('rankeverything_comments');
        location.reload();
    }
}
